...
interface Order {
  ...
  table_number: number;
  waiter_name: string;
  status: 'new' | 'ready' | 'completed' | 'cancelled' | 'preparing';
  created_at: string;
      ...
      <div className="p-3 border-b border-slate-100 flex justify-between items-start">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <StatusBadge status={order.status as any} className={cn(
        ...
        <div className="text-right">
          <div className="text-sm font-bold text-slate-700">Tb {order.table_number}</div>
          <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{order.group_name || 'Group A'}</div>
